...
//
//
//  def make(initial: List[Movie], transactor: Transactor[ExampleTask]): ZLayer[Any, Nothing, ExampleService] = ZLayer.fromEffect {
//
//      for {
...
//      } yield new Service {
//
//        def getMovies(): ExampleTask[List[MovieM]] = new LiveMovieService[ExampleTask](transactor).getMovies().foldM(err => ZIO.fail(err), b =>
//          ZIO.succeed(b.map(c =>
...
//                       year: String,
//                       description: String,
//                       genre: String): ExampleTask[Option[String]] =
//          new LiveMovieService[ExampleTask](transactor).createMovie(MovieName(name), MovieYear(year), MovieDescription(description), stringToGenre(genre))
//            .foldM(
//              err => ZIO.fail(err),
