...
final class HttpApi[F[_]: Concurrent : Timer] private (
    ...
    val reviewRoutes = new TrackRoutes[F](algebras.reviews).routes(userMiddleware)

    val adminMovieRoutes = new AdminMovieRoutes[F](algebras.movies).routes(adminMiddleware)
    val adminReviewRoutes = new adminReviewRoutes[F](algebras.review).routes(adminMiddleware)
...
object ErrorHandler {
  import app.vizion.minervaCore.controllers.Errors._ 
